...
class Project(BaseProject):
    ...
    @property
    def land_slug(self) -> str:
        from django.utils.text import slugify

        if self.land_type == AdminRef.NATION:
            return "france"
        return f"{self.land_id}-{slugify(self.name)}"
    ...
    def get_absolute_url(self):
        return reverse("project:home", kwargs={"land_type": self.land_type_slug, "land_slug": self.land_slug})
