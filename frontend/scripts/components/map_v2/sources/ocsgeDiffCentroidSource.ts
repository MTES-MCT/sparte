...
export class OcsgeDiffCentroidSource extends BaseSource implements SourceInterface {
    ...
    async setMillesimes(newStartIndex: number, newEndIndex: number, newDepartement?: string): Promise<void> {
        if (!this.map || !this.sourceId) {
            console.warn('OcsgeDiffCentroidSource: map ou sourceId non attaché');
            return;
        }

        if (newEndIndex - newStartIndex !== 1) {
            throw new Error("Les millésimes doivent être consécutifs pour la source de différence centroid");
        }
