...
const Connector = styled.div<{ $invisible?: boolean }>`
  ...
  height: 2px;
  background: ${(props) =>
    props.$invisible ? "transparent" : "var(--border-default-grey)"};
  position: relative;
  align-self: center;
  flex-shrink: 0;
...
const HierarchyCard = styled(BaseCard)<{ $isCurrent?: boolean }>`
...
const EmptyHierarchyCard = styled(BaseCard)<{ $isCurrent?: boolean }>`
  padding: 1rem 0.75rem;
...
const StatusBadge = styled.span<{ $status: "success" | "pending" | "waiting" }>`
  ...
  background: ${(props) => {
    switch (props.$status) {
      case "success":
        return "var(--background-contrast-success)";
      case "pending":
        return "var(--background-contrast-warning)";
      case "waiting":
        return "var(--background-contrast-warning)";
    }
  }};
  color: ${(props) => {
    switch (props.$status) {
      ...
      case "pending":
        return "var(--text-default-warning)";
      case "waiting":
        return "var(--text-default-warning)";
    }
  }};
...
const FRANCE_ITEM: TerritorialisationHierarchyItem = {
  ...
  document_url:
    "https://www.legifrance.gouv.fr/jorf/id/JORFTEXT000049676333#:~:text=Pour%20tenir%20compte,p%C3%A9riode%202011%2D2021.",
  document_comment: null,
  is_in_document: true,
};

export const TrajectoiresHierarchy: React.FC<TrajectoiresHierarchyProps> = ({
  hierarchy,
  land_id,
  land_type,
  land_name,
  ...
  return (
    <Container>
      <Header>
        <i
          className="bi bi-diagram-3 fr-text-action-high--blue-france"
          style={{ fontSize: "1.25rem" }}
        />
        <Title>Territorialisation des objectifs</Title>
      </Header>
      <Subtitle>
        Pour {land_name}, les objectifs de réduction de la consommation
        d'espaces NAF sont déclinés de la façon suivantes :
      </Subtitle>
      <TimelineContainer>
        {fullHierarchy.map((item, index) => {
          ...
          return (
            <React.Fragment key={item.land_id}>
              {index > 1 && <Connector />}
              <TimelineItem>
                <HierarchyCard $isCurrent={isCurrent}>
                  <TerritoryName>{item.land_name}</TerritoryName>
                  <ObjectifValue>-{item.objectif}%</ObjectifValue>
                  <ObjectifLabel>
        ...
        {is_from_parent && (
          <>
            <Connector />
            <TimelineItem>
              <EmptyHierarchyCard $isCurrent>
                <TerritoryName>{land_name}</TerritoryName>
                <ObjectifLabel>
      ...
      {is_from_parent && parent_land_name && objectif !== null && (
        <InheritedNotice>
          ...
          <NoticeText>
            <strong>{land_name}</strong> ne dispose pas d'un objectif de
            réduction territorialisé propre. L'objectif affiché (
            <strong>-{objectif}%</strong>) est celui défini pour{" "}
            <strong>{parent_land_name}</strong>, le territoire de niveau
            supérieur dans la chaîne de territorialisation.
          </NoticeText>
        </InheritedNotice>
      )}
        ...
        <MapSection $isVisible={showMap}>
          ...
          <GuideContainer>
            <GuideContent title="Lecture de la carte">
              <p>
                Chaque territoire est coloré selon son objectif de réduction de
                consommation d'espaces.
              </p>
              <p>
                Plus la couleur est foncée, plus l'objectif de réduction est
                ambitieux.
              </p>
              <p>
                Survolez un territoire pour consulter son objectif de réduction.
              </p>
            </GuideContent>
          </GuideContainer>
        </MapSection>
