...
import { COUVERTURE_COLORS, USAGE_COLORS, ARTIFICIALISATION_MATRIX, ALL_OCSGE_COUVERTURE_CODES, ALL_OCSGE_USAGE_CODES } from "../../constants/ocsge_nomenclatures";
...
const MiniMatrixCell = styled.div<{ $artif: boolean; $variant: "before" | "after" | "none" }>`
	aspect-ratio: 1;
	border-radius: 1px;
...
export const ArtificialisationDiffSidePanel: React.FC<ArtificialisationDiffSidePanelProps> = ({
	feature,
	isLocked,
	onClose,
}) => {
	...
	const isArtificial = properties.new_is_artificial === true || properties.new_is_artificial === "true";
	const isDesartificial = properties.new_not_artificial === true || properties.new_not_artificial === "true";
	...
	const oldArtifByMatrix = hasMatrix && ((ARTIFICIALISATION_MATRIX as Record<string, string[]>)[csOld]?.includes(usOld) ?? false);
	const newArtifByMatrix = hasMatrix && ((ARTIFICIALISATION_MATRIX as Record<string, string[]>)[csNew]?.includes(usNew) ?? false);
	const sameMatrixClass = oldArtifByMatrix === newArtifByMatrix;
	const isBySeuil = hasMatrix && (isArtificial || isDesartificial) && sameMatrixClass;
				...
				<Section>
					<InfoRow>
						...
						<InfoValue>
							<Badge
								noIcon
								severity={isArtificial ? "error" : isDesartificial ? "success" : "info"}
								small
							>
								{isArtificial ? "Artificialisation" : isDesartificial ? "Désartificialisation" : "Inconnu"}
							</Badge>
						</InfoValue>
					</InfoRow>
					<InfoRow>
				...
				{isBySeuil && (
					<div className="fr-alert fr-alert--warning fr-alert--sm">
						<p className="fr-text--xs fr-mb-0">
							{`Les couvertures/usages avant et après sont tous deux classés « ${oldArtifByMatrix ? "artificialisé" : "non artificialisé"} » par la matrice OCS GE. Cette ${isArtificial ? "artificialisation" : "désartificialisation"} est détectée par les `}
						...
						<Section>
							...
							<MiniMatrixWrapper ref={wrapperRef}>
									...
									<MiniMatrixGrid>
										...
										{ALL_OCSGE_COUVERTURE_CODES.map(cs => (
											<MiniMatrixHeaderCell
												...
												data-tooltip-id="diff-matrix-tooltip"
										...
										{ALL_OCSGE_USAGE_CODES.map(us => (
											<React.Fragment key={us}>
												...
												{ALL_OCSGE_COUVERTURE_CODES.map(cs => {
													...
													const artif = ARTIFICIALISATION_MATRIX[cs]?.includes(us) ?? false;
													return (
														<MiniMatrixCell
															key={`${cs}-${us}`}
															...
															$variant={variant}
															data-tooltip-id="diff-matrix-tooltip"
															data-tooltip-html={`<span style="display:inline-block;width:8px;height:8px;background:${(COUVERTURE_COLORS as Record<string, string>)[cs] || '#ccc'};margin-right:4px;vertical-align:middle;border-radius:1px"></span>${getCouvertureLabel(cs)}<br/><span style="display:inline-block;width:8px;height:8px;background:${(USAGE_COLORS as Record<string, string>)[us] || '#ccc'};margin-right:4px;vertical-align:middle;border-radius:1px"></span>${getUsageLabel(us)}<br/><b>${artif ? "Artificialisé" : "Non artificialisé"}</b>${isBefore ? "<br/><i>Avant</i>" : ""}${isAfter ? "<br/><i>Après</i>" : ""}`}
														/>
													);
												})}
											</React.Fragment>
								...
								{!isSameCell && (
									<ArrowSvg>
										<defs>
											<marker id="diff-arrowhead" markerWidth="6" markerHeight="5" refX="5" refY="2.5" orient="auto">
										...
										<line
											...
											stroke="#000"
											strokeWidth="1.5"
											markerEnd="url(#diff-arrowhead)"
										/>
									</ArrowSvg>
								)}
								<Tooltip id="diff-matrix-tooltip" className="fr-text--xs" />
								<MiniMatrixLegend>
