export default class Filter {
  ...
  setFilter() {
    switch (this.type) {
      ...
      default:
        console.log(
          `Unknow filter type '${this.type}' for layer '${this.name}'`,
        );
    }
  }
  ...
  CreateFilterVisibility() {
    ...
    button.innerHTML = `<i class="bi ${
      this.value ? "bi-eye" : "bi-eye-slash"
    }"></i>`;
  ...
  CreateFilterSelect() {
    ...
    select.addEventListener("change", (_event) => {
      ...
      this.triggerFilters(
        this.options.find((_obj) => _obj.value === this.value)["data-value"],
      );
    });
  ...
  triggerFilters(_value) {
    this.triggers.forEach((_trigger) => {
      _trigger.items.map((_obj) =>
        this[`${_trigger.method}`](_obj, _trigger.property, _value),
      );
    });
  }
