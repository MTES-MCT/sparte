...
export default class FilterGroup {
  ...
  setFilterGroup() {
    ...
    this.buttonNode.onclick = () => {
      const expanded =
        this.groupHeaderNode.getAttribute("aria-expanded") === "true" || false;
  ...
  togglePlaceholder() {
    const source = this.mapLibre.sources.find(
      (_obj) => _obj.key === this.source,
    );
    if (
      isInRange(this.mapLibre.map.getZoom(), source.minZoom, source.maxZoom)
    ) {
