...
export function formatData(type, _options, _value) {
  const formatter = {
    number: () =>
      new Intl.NumberFormat(_options[0], {
        style: _options[1],
        unit: _options[2],
        maximumFractionDigits: _options[3],
      }).format(_value),
  };
...
export function createDonutChart(colors, counts, formatter) {
  ...
  html += `<circle cx="${r}" cy="${r}" r="${r0}" fill="white" /><text dominant-baseline="central" transform="translate(${r}, ${r})">${formatData(
    formatter[0],
    formatter[1],
    total,
  )}</text></svg></div>`;
