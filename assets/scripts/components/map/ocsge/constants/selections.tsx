...
import { RGBColor, getCouvertureColor, getUsageColor, getCombinedColor } from "./colors"
...
export const newSelections = [
    new Selection({
        ...
        matrix: couvertures.map((couverture): MatrixSelection => ({
            usage: null,
        ...
        Description: () => (
            <p>
            La représentation par <strong>couverture</strong> permet de visualiser les 14 postes de couverture du sol
            définit par l'OCS GE. <a target='_blank' href="https://storymaps.arcgis.com/stories/193550c4e4af4f92845201d74ca8a002">
    ...
    new Selection({
        ...
        matrix: usages.map((usage): MatrixSelection => ({
            couverture: null,
        ...
        Description: () => (
            <p>
            La représentation par <strong>usage</strong> permet de visualiser les 17 postes d'usage du sol
            définit par l'OCS GE. <a target='_blank' href="https://storymaps.arcgis.com/stories/193550c4e4af4f92845201d74ca8a002">
    ...
    new Selection({
        ...
        matrix: couvertures.flatMap(couverture => usages.map(usage => ({
            couverture,
            usage,
            ...
            if (['CS2.2.1', 'CS2.2.2'].includes(couverture) && [
                "US2",
                "US3",
                "US5",
                "US235",
    ...
    new Selection({
        ...
        Description: () => (
            <div>
                <p>247 combinaisons possibles entre les couvertures et les usages du sol</p>
            </div>
        )
    }),
]
...
export const checkIfMatrixSelectionIsInUserFilters = (matrixSelection: MatrixSelection, userFilters: UserFilter[]): boolean => {
    return userFilters.some(({ couverture, usage }) => {
        if (couverture && usage) {
            return couverture === matrixSelection.couverture && usage === matrixSelection.usage
        } else if (couverture) {
