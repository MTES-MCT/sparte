...
const TopBar: React.FC = () => {
  ...
  const formattedDate = useMemo(
    () => formatDateTime(new Date(memoizedProjectData?.created_date)),
    [memoizedProjectData?.created_date],
  );
  const location = useLocation();
  const pathsToHidePeriod = ["vacance-des-logements"];
  const shouldDisplayPeriod = !pathsToHidePeriod.some((path) =>
      ...
      <ItemContainer>
        {shouldDisplayPeriod && (
          <>
            <Item>
              <ItemTitle>
                <i className="bi bi-calendar4-range"></i> Période d'analyse
              </ItemTitle>
              <ItemContent>
                De {memoizedProjectData?.analyse_start_date} à{" "}
                {memoizedProjectData?.analyse_end_date}
                {urls && (
                  <button
                  ...
                  >
                    <span
                      className="fr-icon-pencil-fill fr-icon--sm"
                      aria-hidden="true"
                    ></span>
                  </button>
                )}
        ...
        <Item>
          <ItemTitle>
            <i className="bi bi-bullseye"></i> Maille d'analyse
          </ItemTitle>
