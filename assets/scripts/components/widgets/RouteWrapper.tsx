...
import ConsoCorrectionStatus, {
  ConsoCorrectionStatusEnum,
} from "@components/widgets/ConsoCorrectionStatus";
...
const RouteWrapper: React.FC<RouteWrapperProps> = ({
  ...
  const shouldDisplayOcsgeStatus =
    ocsgeStatus !== undefined && ocsgeStatus !== "COMPLETE_UNIFORM";
  const shouldDisplayGpuStatus = hasGpu !== undefined && hasGpu === false;
  const shouldDisplayConsoCorrectionStatus =
    consoCorrectionStatus !== undefined &&
    consoCorrectionStatus !== ConsoCorrectionStatusEnum.UNCHANGED;
  const shouldDisplayConsoChildren =
    consoCorrectionStatus === undefined ||
    [
      ConsoCorrectionStatusEnum.UNCHANGED,
      ConsoCorrectionStatusEnum.FUSION,
    ].includes(consoCorrectionStatus);
  const shouldDisplayLogementVacantStatus =
    logementVacantStatus !== undefined && logementVacantStatus === false;

  const shouldDisplayChildren =
    !shouldDisplayOcsgeStatus &&
      ...
      <div className="fr-container--fluid fr-p-3w">
        <div className="fr-grid-row">
          <div className="fr-col-12">
            ...
            {shouldDisplayConsoCorrectionStatus && (
              <ConsoCorrectionStatus status={consoCorrectionStatus} />
            )}
