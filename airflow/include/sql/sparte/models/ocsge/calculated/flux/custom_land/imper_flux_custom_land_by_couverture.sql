...
SELECT
    ...
    year_new_index,
    couverture,
    sum(imper_flux_commune_by_couverture.flux_imper) as flux_imper,
    sum(imper_flux_commune_by_couverture.flux_desimper) as flux_desimper,
    sum(imper_flux_commune_by_couverture.flux_imper_net) as flux_imper_net,
    sum(commune.surface) as surface,
    commune.departement
FROM
    {{ ref('imper_flux_commune_by_couverture') }}
INNER JOIN
    {{ ref('commune_custom_land') }} clc
    ON imper_flux_commune_by_couverture.commune_code = clc.commune_code
LEFT JOIN
    {{ ref('commune') }}
    ON imper_flux_commune_by_couverture.commune_code = commune.code
WHERE
    clc.custom_land_id IS NOT NULL
GROUP BY
    clc.custom_land_id, couverture, year_old, year_new, year_old_index, year_new_index, commune.departement
)
SELECT
