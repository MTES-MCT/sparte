...
FROM {{ ref('imper_flux_commune_by_couverture') }}
UNION ALL
SELECT
    code as land_id,
    '{{ var('EPCI') }}' as land_type,
    {{ common_fields | join(", ") }}
FROM {{ ref('imper_flux_epci_by_couverture') }}
UNION ALL
SELECT
    code as land_id,
    '{{ var('DEPARTEMENT') }}' as land_type,
    {{ common_fields | join(", ") }}
FROM {{ ref('imper_flux_departement_by_couverture') }}
UNION ALL
SELECT
    code as land_id,
    '{{ var('REGION') }}' as land_type,
    {{ common_fields | join(", ") }}
FROM {{ ref('imper_flux_region_by_couverture') }}
UNION ALL
SELECT
    code as land_id,
...
SELECT
    ...
    {{ common_fields | join(", ") }}
FROM {{ ref('imper_flux_nation_by_couverture') }}
