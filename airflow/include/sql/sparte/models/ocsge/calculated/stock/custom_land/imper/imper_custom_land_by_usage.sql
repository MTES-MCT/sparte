...
with without_percent as (
    SELECT
        ...
        year,
        usage,
        sum(imper_commune_by_usage.surface) as surface,
        sum(commune.surface) as land_surface,
        commune.departement,
        imper_commune_by_usage.index
    FROM
        {{ ref('imper_commune_by_usage') }}
    INNER JOIN
        {{ ref('commune_custom_land') }} clc
        ON imper_commune_by_usage.code = clc.commune_code
    LEFT JOIN
        {{ ref('commune') }}
        ON imper_commune_by_usage.code = commune.code
    WHERE
        clc.custom_land_id IS NOT NULL
    GROUP BY
        clc.custom_land_id, usage, year, commune.departement, imper_commune_by_usage.index
)
SELECT
