/*
Ce test vérifie que la table for_vector_tiles_difference_ocsge_centroid
contient le même nombre d'objets que la table for_vector_tiles_difference_ocsge.
...
with
    count_centroid as (
        ...
        from {{ ref("for_vector_tiles_difference_ocsge_centroid") }}
    ),
    count_normal as (
        select count(*) as nb_normal
        from {{ ref("for_vector_tiles_difference_ocsge") }}
    ),
    comparison as (
        select
